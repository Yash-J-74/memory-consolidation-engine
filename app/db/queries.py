...
def log_consolidation_decision(conn: sqlite3.Connection, log: ConsolidationLog):
    ...
    (id, user_id, new_memory_id, decision, existing_memory_id, similarity_score, llm_called, reasoning, extraction_index)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    conn.execute(query, (
        log.id, log.user_id, log.new_memory_id, log.decision, log.existing_memory_id,
        log.similarity_score, log.llm_called, log.reasoning, log.extraction_index
    ))
...
def insert_session(conn: sqlite3.Connection, session: Session):
    ...
    (id, user_id, raw_text, extracted_count, add_count, noop_count, update_count, conflict_count, skipped_count)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    conn.execute(query, (
        ...
        session.conflict_count, session.skipped_count
    ))
